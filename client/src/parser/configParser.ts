...
function mapWirelessVAPs(section: RawSection): WirelessVAP[] {
  return section.entries.map((e) => {
    const p = e.properties;
    const trafficMode: WirelessVAP['trafficMode'] = bool(p['mesh-backhaul']) ? 'mesh' : bool(p['local-bridging']) ? 'bridge' : 'tunnel';
    return {
      name: e.name,
      ...
      vrf: num(p['vrf']),
      vlanid: num(p['vlanid']),
      addressingMode: str(p['address-mode'], 'manual') as WirelessVAP['addressingMode'],
      ip: str(p['ip']),
      allowaccess: strArr(p['allowaccess']),
      deviceDetection: bool(p['device-identification'], true),
      explicitWebProxy: bool(p['explicit-web-proxy']),
      broadcast: bool(p['broadcast-ssid'], true),
      maxClients: num(p['max-clients']),
      beaconAdvertising: strArr(p['beacon-advertising']),
      securityMode: str(p['security'], 'open'),
      captivePortal: bool(p['captive-portal']) || str(p['security']) === 'captive-portal',
      passphrase: str(p['passphrase']),
      pskMode: 'single',
      authServer: str(p['auth']),
      macFilter: bool(p['mac-filter']),
      macFilterPolicy: str(p['mac-filter-policy-other'], 'disable') as WirelessVAP['macFilterPolicy'],
      radiusMacAuth: bool(p['radius-mac-auth']),
      ...
      scanBotnet: str(p['scan-botnet-connections'], 'disable') as WirelessVAP['scanBotnet'],
      utmLog: bool(p['utm-log'], true),
      schedule: str(p['schedule'], 'always'),
      blockIntraVap: bool(p['intra-vap-privacy']),
      broadcastSuppression: strArr(p['broadcast-suppression']),
      quarantine: bool(p['quarantine']),
      vlanPooling: (() => { const vp = str(p['vlan-pooling']); return vp === '' || vp === 'disable' ? '' : vp; })(),
      nac: bool(p['nac']),
      nacProfile: str(p['nac-profile']),
      localStandalone: bool(p['local-standalone']),
      localAuthentication: bool(p['local-authentication']),
